...
base_y = 0.148661704
base_z = 0.0952328317
# Sweep rightward in y (descending). linspace avoids arange endpoint quirks.
y_candidates = np.linspace(0.14, 0.10, 5)
ik_seed = np.array(left_arm_pos, dtype=np.float64)
last_reachable_y = None
reachable_ys = []

print("----IK Y sweep----")
for y in y_candidates:
    ik_input = KinGroupIKInput()
    ik_input.pose = Isometry3d.Identity() * Translation3d(base_x, float(y), base_z) * Quaterniond(R)
    ik_input.tip_link_name = "left_wrist_yaw_link"
    ik_input.working_frame = "torso_link"
    ik_inputs = KinGroupIKInputs()
    ik_inputs.append(ik_input)

    ik_solutions = kg.calcInvKin(ik_inputs, ik_seed)
    reachable = len(ik_solutions) > 0
    print(f"y={y:.2f} -> IK {'OK' if reachable else 'FAIL'} (n={len(ik_solutions)})")
    if reachable:
        last_reachable_y = float(y)
        reachable_ys.append(float(y))

if last_reachable_y is None:
    raise RuntimeError("No reachable y found in sweep range; widen range or change orientation.")

# Do not plan at the exact boundary; use a small safety margin.
if len(reachable_ys) >= 2:
    target_y = reachable_ys[-2]
else:
    target_y = reachable_ys[-1]
print(f"Using boundary-safe y from sweep: {target_y:.2f}")
...
#------------------------------------------------------------------------------------
wp1 = CartesianWaypoint(Isometry3d.Identity() * Translation3d(base_x, base_y, base_z) * Quaterniond(R))
wp2 = CartesianWaypoint(Isometry3d.Identity() * Translation3d(0.3, base_y, 0.05) * Quaterniond(R))
wp3 = CartesianWaypoint(Isometry3d.Identity() * Translation3d(0.3, base_y, 0.2) * Quaterniond(R))
